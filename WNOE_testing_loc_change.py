...
This code is for the Wavelet Neural Operator - elastography for Gaussian with location change dataset.
"""
...
PATH = 'data/DATA_Gaussian_loc_change.mat'
ntrain = 440
ntest = 60
...
model = torch.load('model/model_Gaussian_loc_change')
...
for value in range(y_test.shape[0]):
    if value % 17 == 1:
        ...
        plt.imshow(u_x.cpu()[value,:,:], cmap='gist_ncar', extent=[0,20,0,20],
        ...
        plt.imshow(u_y.cpu()[value,:,:], cmap='gist_ncar', extent=[0,20,0,20],
        ...
        plt.imshow(actual.cpu()[value,:,:], cmap='gist_ncar',
        ...
        plt.imshow(pred.cpu()[value,:,:], cmap='gist_ncar',
                    extent=[0,20,0,20], interpolation='Gaussian', vmin=0.01, vmax=0.015)
        plt.xlabel('x'); plt.ylabel('y'); plt.colorbar(fraction=0.046)
        if index == 1:
        ...
        plt.imshow(torch.abs(pred[value,:,:]-actual[value,:,:])/torch.abs(actual[value,:,:]).cpu(),
                    cmap='gist_ncar', extent=[0,20,0,20], interpolation='Gaussian',
                    vmin=0, vmax=1e-2)
        plt.xlabel('x'); plt.ylabel('y'); plt.colorbar(fraction=0.046)
        if index == 1:
...
# figure1.savefig('center_change_51.pdf', format='pdf', dpi=500, bbox_inches='tight')

...
class WaveConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        dwt = DWT(J=self.level, mode='symmetric', wave='db6').to(x.device) # db6
        x_ft, x_coeff = dwt(x)
...
"""
# torch.save(model, 'model/data_all_1900samples_review')
# scipy.io.savemat('pred/data_all_1900samples_review.mat', mdict={'pred': pred.cpu().numpy()})

# scipy.io.savemat('epoch_loss/epoch_loss_all_1900samples_review.mat', mdict={'epoch_loss': epoch_loss.cpu().numpy()})
# scipy.io.savemat('epoch_prediction/epoch_pred_all_1900samples_review.mat', mdict={'epoch_pred': epoch_pred.cpu().numpy()})

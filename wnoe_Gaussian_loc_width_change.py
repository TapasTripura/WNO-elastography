...
# # Adding noise: # if required, add noise
# u_x = u_x + torch.randn(u_x.shape)*torch.std(u_x,0)*0.15
# u_y = u_y + torch.randn(u_y.shape)*torch.std(u_x,0)*0.15

# snr1 = 20*np.log10(torch.norm(u_x, p=2)/torch.norm(torch.randn(u_x.shape)*torch.std(u_x,0)*0.01)) # 68.2721
# snr2 = 20*np.log10(torch.norm(u_x, p=2)/torch.norm(torch.randn(u_x.shape)*torch.std(u_x,0)*0.02)) # 62.2502
# snr5 = 20*np.log10(torch.norm(u_x, p=2)/torch.norm(torch.randn(u_x.shape)*torch.std(u_x,0)*0.05)) # 54.2822
# snr10 = 20*np.log10(torch.norm(u_x, p=2)/torch.norm(torch.randn(u_x.shape)*torch.std(u_x,0)*0.10)) # 48.2292
# snr15 = 20*np.log10(torch.norm(u_x, p=2)/torch.norm(torch.randn(u_x.shape)*torch.std(u_x,0)*0.15)) # 44.6540
...
for ep in range(epochs):
    ...
    epoch_pred[ep] = out[9,:,:].detach().cpu()
    test_l2 /= ntest
    t2 = default_timer()
...
"""
# torch.save(model, 'model/data_center_width_noise15%')
# scipy.io.savemat('pred/data_center_width_noise15%.mat', mdict={'pred': pred.cpu().numpy()})

# scipy.io.savemat('epoch_loss/epoch_loss_center_width_noise15%.mat', mdict={'epoch_loss': epoch_loss.cpu().numpy()})
# scipy.io.savemat('epoch_prediction/epoch_pred_center_width_noise15%.mat', mdict={'epoch_pred': epoch_pred.cpu().numpy()})

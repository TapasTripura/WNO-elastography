...
This code is for the Wavelet Neural Operator - elastography for Inclusion distribution.
"""
...
class WaveConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, level, dummy):
        ...
        self.level = level
        dwt_ = DWT(J=self.level, mode='zero', wave='db4').to(dummy.device)
    ...
    def forward(self, x):
        ...
        dwt = DWT(J=self.level, mode='zero', wave='db4').to(x.device)
        x_ft, x_coeff = dwt(x)
        ...
        idwt = IDWT(mode='zero', wave='db4').to(x.device)
...
model = torch.load('model/model_inclusion_tension')
...
for value in range(y_test.shape[0]):
    if value % 17 == 1:
        ...
        plt.imshow(u_x.cpu()[value,:,:], cmap='turbo', extent=[0,20,0,20],
        ...
        plt.imshow(u_y.cpu()[value,:,:], cmap='turbo', extent=[0,20,0,20],
        ...
        plt.imshow(actual.cpu()[value,:,:], cmap='turbo',
                    extent=[0,20,0,20], interpolation='Gaussian', vmin=0, vmax=1)
        plt.xlabel('x'); plt.ylabel('y'); plt.colorbar(fraction=0.046)
        if index == 1:
            plt.text(-9,5,'E (Actual)', rotation=90, fontsize=30, color='m', fontweight='bold')
        
        plt.subplot(5,4, index+12)
        plt.imshow(pred.cpu()[value,:,:], cmap='turbo',
                    extent=[0,20,0,20], interpolation='Gaussian', vmin=0, vmax=1)
        plt.xlabel('x'); plt.ylabel('y'); plt.colorbar(fraction=0.046)
        if index == 1:
        ...
        plt.imshow(torch.abs(pred[value,:,:]-actual[value,:,:])/torch.abs(actual[value,:,:]).cpu(),
                    cmap='turbo', extent=[0,20,0,20], interpolation='Gaussian',
                    vmin=0, vmax=1e-2)
        plt.xlabel('x'); plt.ylabel('y'); plt.colorbar(fraction=0.046)
        if index == 1:
...
# figure1.savefig('inclusion_51.pdf', format='pdf', dpi=500, bbox_inches='tight')

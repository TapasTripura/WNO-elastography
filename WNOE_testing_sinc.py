...
This code is for the Wavelet Neural Operator - elastography for Sinc dataset.
"""
...
class WaveConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, level, dummy):
        ...
        self.level = level
        dwt_ = DWT(J=self.level, mode='zero', wave='db6').to(dummy.device)
    ...
    def forward(self, x):
        ...
        dwt = DWT(J=self.level, mode='zero', wave='db6').to(x.device)
        x_ft, x_coeff = dwt(x)
        ...
        idwt = IDWT(mode='zero', wave='db6').to(x.device)
...
x_grid = reader.read_field('DATA1')[::2,::2,0,:]
y_grid = reader.read_field('DATA1')[::2,::2,1,:]
u_x = reader.read_field('DATA1')[::2,::2,2,:]
u_y = reader.read_field('DATA1')[::2,::2,3,:]
E_final = reader.read_field('DATA1')[::2,::2,-1,:]
...
model = torch.load('model/model_sinc')
...
for kk in range(len(idx)):
    ...
    plt.imshow(u_x.cpu()[value,:,:], cmap='seismic', extent=[0,20,0,20],
    ...
    plt.imshow(u_y.cpu()[value,:,:], cmap='seismic', extent=[0,20,0,20],
                interpolation='Gaussian', vmin=0, vmax=0.5)
    plt.xlabel('x'); plt.ylabel('y'); plt.colorbar(fraction=0.046)
    if index == 1:
        plt.text(-9,8,'$u_{yy}$', rotation=90, fontsize=30, color='c', fontweight='bold')
    
    plt.subplot(5,4, index+8)
    plt.imshow(actual.cpu()[value,:,:], cmap='seismic',
                extent=[0,20,0,20], interpolation='Gaussian', vmin=0.2, vmax=1.2)
    plt.xlabel('x'); plt.ylabel('y'); plt.colorbar(fraction=0.046)
    if index == 1:
        plt.text(-9,5,'E (Actual)', rotation=90, fontsize=30, color='m', fontweight='bold')
    
    plt.subplot(5,4, index+12)
    plt.imshow(pred.cpu()[value,:,:], cmap='seismic',
                extent=[0,20,0,20], interpolation='Gaussian', vmin=0.2, vmax=1.2)
    plt.xlabel('x'); plt.ylabel('y'); plt.colorbar(fraction=0.046)
    if index == 1:
    ...
    plt.imshow(torch.abs(pred[value,:,:]-actual[value,:,:])/torch.abs(actual[value,:,:]).cpu(),
                cmap='seismic', extent=[0,20,0,20], interpolation='Gaussian',
                vmin=0, vmax=1e-1)
    plt.xlabel('x'); plt.ylabel('y'); plt.colorbar(fraction=0.046)
    if index == 1:
...
# figure1.savefig('sinc_51.pdf', format='pdf', dpi=300, bbox_inches='tight')

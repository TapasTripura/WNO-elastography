...
This code is for the Wavelet Neural Operator - elastography for sinusoidal distribution.
"""
...
model = torch.load('model/model_sinusoidal')
...
for value in range(y_test.shape[0]):
    if value % 15 == 1:
        ...
        plt.imshow(u_x.cpu()[value,:,:], cmap='nipy_spectral', extent=[0,20,0,20],
        ...
        plt.imshow(u_y.cpu()[value,:,:], cmap='nipy_spectral', extent=[0,20,0,20],
        ...
        plt.imshow(actual.cpu()[value,:,:], cmap='nipy_spectral',
        ...
        plt.imshow(pred.cpu()[value,:,:], cmap='nipy_spectral',
                    extent=[0,20,0,20], interpolation='Gaussian', vmin=0, vmax=1)
        plt.xlabel('x'); plt.ylabel('y'); plt.colorbar(fraction=0.046)
        if index == 1:
        ...
        plt.imshow(torch.abs(pred[value,:,:]-actual[value,:,:])/torch.abs(actual[value,:,:]).cpu(),
                    cmap='nipy_spectral', extent=[0,20,0,20], interpolation='Gaussian',
                    vmin=0, vmax=1e-2)
        plt.xlabel('x'); plt.ylabel('y'); plt.colorbar(fraction=0.046)
        if index == 1:
...
# figure1.savefig('wavy_51.pdf', format='pdf', dpi=500, bbox_inches='tight')

...
for ep in range(epochs):
    ...
    epoch_pred[ep] = out[-3,:,:].detach().cpu()
    test_l2 /= ntest
    t2 = default_timer()
...
"""
# torch.save(model, 'model/data_center_sinc_500samples')
# scipy.io.savemat('pred/data_center_sinc_500samples.mat', mdict={'pred': pred.cpu().numpy()})

# scipy.io.savemat('epoch_loss/epoch_loss_center_sinc_500samples.mat', mdict={'epoch_loss': epoch_loss.cpu().numpy()})
# scipy.io.savemat('epoch_prediction/epoch_pred_center_sinc_500samples.mat', mdict={'epoch_pred': epoch_pred.cpu().numpy()})

...
# noise = 0.1
# u_x = u_x + torch.randn(u_x.shape)*torch.std(u_x,0)*noise
# u_y = u_y + torch.randn(u_y.shape)*torch.std(u_x,0)*noise

# snr1 = 20*np.log10(torch.norm(u_x, p=2)/torch.norm(torch.randn(u_x.shape)*torch.std(u_x,0)*noise))

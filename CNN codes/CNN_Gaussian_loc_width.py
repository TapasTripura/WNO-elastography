...
This code is for the BLOCK CNN implementation for Gaussian distribution with random location and width.
"""
...
# u_x = u_x + torch.randn(u_x.shape)*torch.std(u_x,0)*0.01
# u_y = u_y + torch.randn(u_y.shape)*torch.std(u_x,0)*0.01

# snr1 = 20*np.log10(torch.norm(u_x, p=2)/torch.norm(torch.randn(u_x.shape)*torch.std(u_x,0)*0.01))
# snr2 = 20*np.log10(torch.norm(u_x, p=2)/torch.norm(torch.randn(u_x.shape)*torch.std(u_x,0)*0.02))
# snr5 = 20*np.log10(torch.norm(u_x, p=2)/torch.norm(torch.randn(u_x.shape)*torch.std(u_x,0)*0.05))
# snr10 = 20*np.log10(torch.norm(u_x, p=2)/torch.norm(torch.randn(u_x.shape)*torch.std(u_x,0)*0.10))
